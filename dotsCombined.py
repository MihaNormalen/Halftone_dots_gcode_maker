...
if __name__ == "__main__":
    ...
    BRUSH_WIDTH  = 1.0         # Debelina čopiča (mm)
    OVERLAP      = 0.1         # Prekrivanje (0.0 – 1.0)
    TARGET_WIDTH = 690.0       # Ciljna širina slike (mm)

    # Hitrosti & pospeški
    FEED_TRAVEL  = 12000       # Potovalna hitrost (mm/min)
    FEED_PAINT   = 300         # Hitrost barvanja (mm/min)
    ACCEL_TRAVEL = 12000       # Pospešek pri potovanju (mm/s²)
    ACCEL_PAINT  = 1000        # Pospešek pri barvanju (mm/s²)

    # Doseg čopiča med dopi
    MIN_DIST     = 200.0       # Min pot pred dip-om (mm)
    MAX_DIST     = 240.0       # Max pot pred dip-om (mm)
    ...
    DIP_X        = 104.0
    DIP_Y        = 679.0
    DIP_Z        = 2.3         # Z globina namakanja
    DIP_JITTER   = 10.0        # Naključni odmik namakanja (mm)
    DIP_SPIRAL_LOOPS = 1.0     # Stevilo zavojev spirale pri dipu
    DIP_SPIRAL_R = 10.0        # Radij spirale pri dipu (mm)
    DIP_WIPE_R   = 34.2        # Razdalja brisanja po dipu (mm)

    # Višine Z
    Z_SAFE       = 1.4         # Nizka varna Z (nad površino)
    Z_SAFE_DIP   = 17.0        # Visoka varna Z (nad vsem)
    Z_WIPE_EXIT  = 9.4         # Z pri izhodu iz brisanja
    Z_GLOBAL_OFF = 0.0
    # ==========================================
