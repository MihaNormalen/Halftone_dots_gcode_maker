...
class GCodeBaseGenerator:
    def __init__(self, feed_rate, feed_rate_paint, x_offset, y_offset,
        ...
        self.dip_count = 0
        self.current_pos = (0, 0) # Track current X,Y
    ...
    def _perform_dip(self, target_x=None, target_y=None):
        """
        Perform dip using the EXACT logic from line_conce_var8.py:
        1. Diagonal traverse IN (Lift Z while moving to edge).
        2. Spiral Mix.
        3. Diagonal traverse OUT (Descend Z while moving to target).
        """
        self.gcode.append(f"\n; --- Namakanje (Optimized var8 Style) ---")
        ...
        # --- 1. DIAGONAL TRAVERSE INTO PETRI ---
        ...
        if dist > 0:
            ...
            # Edge point at wipe_r from petri center
            edge_x = self.dip_location[0] - dx * self.dip_wipe_radius
            edge_y = self.dip_location[1] - dy * self.dip_wipe_radius
        else:
            edge_x = self.dip_location[0] + self.dip_wipe_radius
            edge_y = self.dip_location[1]
        
        # CRITICAL MOVE FROM VAR8: Diagonal traverse TO edge
        self.gcode.append(f"G0 X{edge_x:.3f} Y{edge_y:.3f} Z{self.z_safe_dip:.3f}")
        
        # Now safely move to jittered dip position
        self.gcode.append(f"G0 X{active_x:.3f} Y{active_y:.3f}")
        
        # --- 2. RANDOMIZED SPIRAL MIXING ---
        ...
        if target_x is not None and target_y is not None:
            ...
            if dist_out > 0:
                ...
                wipe_edge_x = self.dip_location[0] + self.dip_wipe_radius
                wipe_edge_y = self.dip_location[1]
...
class SVGCircleGenerator(GCodeBaseGenerator):
    def __init__(self, brush_width, overlap, min_distance_per_dip, max_distance_per_dip, fill_type, *args, **kwargs):
        ...
        self.current_pos = (0, 0) # Start tracking from machine zero
    ...
    def run(self, svg_path, target_width):
        ...
        # --- FIX: INITIAL LIFT TO PREVENT DIAGONAL COLLISION ---
        self._set_machine_speed('travel')
        self.gcode.append(f"G0 Z{self.z_safe_dip:.3f}")
...
if __name__ == "__main__":
    FILL_TYPE    = 'concentric' # 'spiral' or 'concentric'
    BRUSH_WIDTH  = 7.0         
    OVERLAP      = 0.2         
    MIN_DIST     = 100.0       
    MAX_DIST     = 150.0       
    ...
    painter = SVGCircleGenerator(
        ...
        dip_wipe_radius=DIP_WIPE_R, z_wipe_travel_raw=1.0, dip_entry_radius=30.0,
